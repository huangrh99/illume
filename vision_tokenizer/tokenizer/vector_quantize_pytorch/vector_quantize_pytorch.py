...
def cdist(x, y):
    ...
    xy = einsum('b i d, b j d -> b i j', x, y.to(x.dtype)) * -2
    ...
    def forward(
        ...
        if self.training:
            ...
            if exists(codebook_transform_fn):
                ...
                quantize = einsum('h b n c, h c d -> h b n d', unpacked_onehot, embed.to(unpacked_onehot))
...
class CosineSimCodebook(Module):
    ...
    def forward(
        ...
        if exists(codebook_transform_fn):
            ...
            dist = einsum('h n d, h c d -> h n c', flatten.to(embed), embed).to(flatten)
