...
try:
    ...
    def slow_attn(query, key, value, scale: float = None, attn_mask=None, dropout_p=0.0):
        if scale is None:
...
class SelfAttentionNoDrop(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.using_flash_attn and qkv.dtype != torch.float32:
            ...
            # , scale = self.scale
            oup = slow_attn(query=q, key=k, value=v).transpose(1, 2).reshape(B, L, C)
